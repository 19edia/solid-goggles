...
export async function renderCard(member,divisionName,{issuedAt=new Date()}={}){
 ...
 const leader=String(member.member_rank||'').toLowerCase().includes('lider');
 const accent=ally?'#e9c385':leader?'#8b7cff':'#a6f0bc';
 const bg=c.createLinearGradient(0,0,1600,1100);bg.addColorStop(0,'#21313b');bg.addColorStop(.6,'#0b151d');bg.addColorStop(1,ally?'#332a20':leader?'#24164a':'#153d32');c.fillStyle=bg;c.fillRect(0,0,1600,1100);
 ...
 c.strokeStyle=leader?'#8b7cff':'#607574';c.lineWidth=2;c.beginPath();c.roundRect(28,28,1544,1044,28);c.stroke();c.strokeStyle='#34484c';c.beginPath();c.roundRect(42,42,1516,1016,22);c.stroke();
 c.fillStyle=accent;c.fillRect(70,65,5,76);c.font='42px IFJBold';c.fillText(leader?'ROKUHARA LEADER':'ROKUHARA',95,98);c.font='17px IFJRegular';c.fillStyle='#a8b8bb';c.fillText('CENTRAL IFJ / UNIÃO • RESPEITO • LEALDADE',97,133);
 c.fillStyle=ally?'#423a29':'#224438';c.beginPath();c.roundRect(1080,66,450,53,26);c.fill();c.font='19px IFJBold';c.fillStyle=accent;c.textAlign='center';c.fillText(ally?'CREDENCIAL DE ALIANÇA':leader?'CARTEIRA DE LÍDER':'IDENTIDADE DE MEMBRO',1305,100);
